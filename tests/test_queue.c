...
int main()
{
  ...
  u16 position = 2;
  TestData.storage_ptr_test_C[position] = q->ops_->dequeue(queue_3, NULL);
  ...
  position++;
  TestData.storage_ptr_test_C[position] = q->ops_->dequeue(queue_3, 2);
  if (NULL == TestData.storage_ptr_test_C[position])
    printf("ERROR: NULL pointer extracted at front queue_3\n");
  else
    printf("extracted \"%s\" at front in queue_3\n", TestData.storage_ptr_test_C[position]);
  ...
  for (u16 i = 0; i < 2; ++i)
  {
    TestData.storage_ptr_test_C[i] = q->ops_->dequeue(queue_3, NULL);
  ...
  for (u16 i = 0; i < kNumberOfStoragePtrTest_C; ++i)
  {
    ...
    {
      data = q->ops_->dequeue(queue_3, NULL);
      MM->free(data);
  ...
  printf("\n\n# Test Extract\n");
  data = q->ops_->dequeue(queue_2, NULL);
  if (NULL != data)
  {
